...
namespace BusinessLogicLayer
{
    public class busEmployees
    {
        ...
        public string CheckToAdd(string CallBy, ref DataTable dtResult, string FullName, string Gender, string BirthDay)
        {
            ...
            try
            {
                daoEmployees dEmployees = new daoEmployees();
                return dEmployees.CheckToAdd(callFrom, ref dtResult, FullName, Gender, BirthDay);
            }
            catch (Exception ex)
            {
        ...
        public string GetAll(string CallBy, ref DataTable dtResult)
        {
            ...
            {
                daoEmployees dEmployees = new daoEmployees();
                return dEmployees.GetAll(callFrom, ref dtResult);
            }
            catch (Exception ex)
            {
                Utilities.Functions.WriteLog(Utilities.Parameters.LogErrorPath, callFrom + ":\n" + ex.Message);
        ...
        public string GetByStatus(string CallBy, ref DataTable dtResult, string StatusID)
        {
            ...
            {
                daoEmployees dEmployees = new daoEmployees();
                return dEmployees.GetByStatus(callFrom, ref dtResult, StatusID);
            }
            catch (Exception ex)
            {
                Utilities.Functions.WriteLog(Utilities.Parameters.LogErrorPath, callFrom + ":\n" + ex.Message);
